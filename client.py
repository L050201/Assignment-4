...
def send_and_receive(socket: socket.socket, address: Tuple[str, int], data: bytes, operation: str) -> Optional[str]:
    ...
    logging.error(f"{operation} Failed: The maximum number of retries")
    return None
...
def download_file(socket: socket.socket, server_address: Tuple[str, int], filename: str) -> bool:
    ...
    if parts[0] == "ERR":
        logging.error(f"Error: The file does not exist - {filename}")
        return False
    ...
    logging.info(f"File size: {file_size} bytes, data ports: {data_port}")
    ...
    try:
        with open(filename, 'wb') as f:
            ...
            while total_received < file_size:
                ...
                if parts[0] != "FILE" or parts[1] != filename or parts[2] != "OK":
                    logging.error(f"Invalid data response: {response}")
                    continue
                # SEND A FILE GET REQUEST TO THE DATA PORT
        ...
        logging.error(f"Error while writing file {filename}: {e}")
        if os.path.exists(filename):
            os.remove(filename)
        return False
...
def main():
    if len(sys.argv) != 4:
        logging.error("usage: python client.py <host name> <Server port> <File list path>")
        sys.exit(1)
    ...
    try:
        ...
        if not os.path.exists(file_list_path):
            logging.error(f"The file list {file_list_path} does not exist.")
            sys.exit(1)
