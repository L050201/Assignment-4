...
def handle_client_request(welcome_socket: socket.socket, request: str, client_address: Tuple[str, int]):
    ...
    logging.info(f"Ready to send the document: {filename} (Size:{file_size}B,Data port : {data_port}, Client: {client_address})")
...
def handle_data_transmission(filename: str, client_address: Tuple[str, int], data_port: int):
    try:
        ...
        logging.info(f"Data thread started: Port {data_port}, Document {filename}")
        with open(filename, 'rb') as f:
            ...
            while True:
                try:
                    ...
                    logging.info(f"Received data request: {request_str} from {client_address}")
                    ...
                    if parts[2] == "CLOSE":
                        ...
                        logging.info(f"File transfer completed: {filename} (Client: {client_address})")
                        break
                    # Handle closure requests
                        ...
                        try:
                            ...
                            if len(file_data) == block_size:
                                ...
                                logging.error(f"Failed to read the data block: Expectation {block_size}B, Actual {len(file_data)}B (location: {start})")
                        except (IndexError, ValueError) as e:
                            logging.error(f"Request parsing failed: {e}, request: {request_str}")
                except socket.timeout:
                    continue
                except Exception as e:
                    logging.error(f"Data transmission anomaly: {e}")
                    break
    except Exception as e:
        logging.error(f"Data thread exception: {e}")
    finally:
        try:
            data_socket.close()
        except:
